...
def save_single_point_outputs(image, dot_mask, center_yx, output_root, image_name, dot_radius=6):
    ...
    Image.fromarray(point_cut).save(output_root / f"{stem}_point_cut2.png")

    overlay = image.copy()
    if center_yx is not None:
        cy, cx = center_yx
        cv2.circle(overlay, (cx, cy), dot_radius, (255, 0, 0), thickness=2)
    Image.fromarray(overlay).save(output_root / f"{stem}_point_overlay2.png")
    ...
    if len(ys) > 0:
        ...
        Image.fromarray(crop).save(output_root / f"{stem}_point_crop2.png")
...
def main():
    parser = argparse.ArgumentParser(
        description="每张图仅绘制一个半径6的点：满足边界距离约束，并可选择符合/不符合 histogram 条件。"
    )
    parser.add_argument(
        "--input",
    ...
    parser.add_argument(
        ...
        help="若设置，则改为选择“不符合 histogram 条件”的点（默认选择符合条件）",
    )
